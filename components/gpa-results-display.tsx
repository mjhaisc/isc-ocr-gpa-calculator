...
import { GraduationCap, School, TrendingUp, Download, AlertTriangle } from "lucide-react"
...
export function GPAResultsDisplay({ results, studentName, gradingScale }: GPAResultsProps) {
  if (!results) return null

  const { institutional, transfer, cumulative, breakdown, insights, warnings } = results
    ...
    <div className="space-y-4">
      ...
      <Card className="bg-gradient-to-r from-blue-50 to-indigo-50">
        <CardHeader>
          ...
          {studentName && <CardDescription>Student: {studentName}</CardDescription>}
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Cumulative GPA */}
            <div className="text-center p-4 bg-white rounded-lg shadow-sm">
              <div className="text-3xl font-bold text-blue-600">{cumulative?.toFixed(2) || "N/A"}</div>
            ...
            {/* Institutional GPA */}
            <div className="text-center p-4 bg-white rounded-lg shadow-sm">
              <div className="text-3xl font-bold text-green-600">{institutional?.toFixed(2) || "N/A"}</div>
            ...
            {/* Transfer GPA */}
            <div className="text-center p-4 bg-white rounded-lg shadow-sm">
              <div className="text-3xl font-bold text-purple-600">{transfer?.toFixed(2) || "N/A"}</div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-semibold">{breakdown.totalCredits}</div>
              <div className="text-sm text-gray-600">Total Credits</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-semibold">{breakdown.institutionalCredits}</div>
              <div className="text-sm text-gray-600">Institutional</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-semibold">{breakdown.transferCredits}</div>
              <div className="text-sm text-gray-600">Transfer</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-semibold">{breakdown.qualityPoints.toFixed(1)}</div>
